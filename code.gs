...
function sendEmail(missing) {
  ...
  var url = "YOUR_WEB_APP_URL_HERE"; // Update this!
  var body = "<h2>약 복용 알림</h2><p>아직 드시지 않은 약: " + missing.join(", ") + "</p>" +
             "<a href='" + url + "'>체크하러 가기</a>";
             
  MailApp.sendEmail({to: email, subject: "💊 약 복용 알림", htmlBody: body});
}
